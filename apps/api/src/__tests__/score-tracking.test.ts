...
describe("score tracking", () => {
  ...
  describe("score awarded in vote endpoint", () => {
    it("awards vote score for new votes only", async () => {
      const clientId = "vote-player";
      const taskId = "task-123";
      let scoreEventInserted = false;

      queryMock.mockImplementation((text: string) => {
        if (text === "BEGIN" || text === "COMMIT") {
          return Promise.resolve({});
        }
        if (text.includes("FROM tasks WHERE task_id") && text.includes("FOR UPDATE")) {
          return Promise.resolve({ rowCount: 1 });
        }
        // First check for existing vote - return empty (new vote)
        ...
        if (text.includes("SUM(weight * EXP")) {
          return Promise.resolve({ rows: [{ vote_score: 1.0 }] });
        }
        if (text.includes("UPDATE tasks AS t")) {
          return Promise.resolve({
            rows: [{
              task_id: taskId,
              status: "pending",
        ...
        if (text.includes("INSERT INTO player_scores")) {
          return Promise.resolve({ rows: [{ total_score: SCORE_ACTIONS.voteSubmitted }] });
        }
        if (text.includes("INSERT INTO score_events")) {
          scoreEventInserted = true;
          return Promise.resolve({});
        }
      ...
      const payload = response.json();
      expect(payload.ok).toBe(true);
      expect(payload.score_awarded).toBe(SCORE_ACTIONS.voteSubmitted);
      expect(scoreEventInserted).toBe(true);

      await app.close();
    });

    it("does not award score for vote changes", async () => {
      const clientId = "vote-player";
      const taskId = "task-123";
      let scoreEventInserted = false;
      ...
      queryMock.mockImplementation((text: string) => {
        ...
        // Return existing vote (vote change, not new)
        if (text.includes("FROM task_votes WHERE task_id") && text.includes("AND client_id")) {
      ...
      const payload = response.json();
      expect(payload.ok).toBe(true);
      expect(payload.score_awarded).toBe(0); // No score for vote change
      expect(scoreEventInserted).toBe(false);
